...
void handle_cmd( const geometry_msgs::Twist& cmd_msg) {
  ...
  if (z == 0) {     // go straight
    ...
    rpm_req1 = cmd_msg.linear.x*60/(pi*wheel_diameter);
...
void loop() {
  ...
  if(time-lastMilli>= LOOPTIME)   {      // enter tmed loop
    ...
    //    rpm_act1 = double((count1-countAnt1)*60*1000);
      //  rpm_act1 = rpm_act1/double((time-lastMilli)*13*30);   
      //  lastMilli = time;
       // countAnt1 = count1;
...
void publishRPM(unsigned long time) {
  ...
  rpm_msg.vector.y = PWM_val1;
 // rpm_msg.vector.z = double(time)/1000;
  rpm_msg.vector.z = rpm_req1;
